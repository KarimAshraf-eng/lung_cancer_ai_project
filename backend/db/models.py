...
class Doctor(Base):
    ...
    # 🔴 علاقة جديدة مع جدول سجل تسجيل الدخول/الخروج
    login_history = relationship("LoginHistory", back_populates="doctor",
...
# ════════════════════════════════════════════════════════════════════
# 🔴 جدول جديد: LoginHistory
# ════════════════════════════════════════════════════════════════════
# بيحتفظ بسجل كامل لكل عمليات الـ login والـ logout اللي بتعملها الدكاترة.
# كل row بيمثل حدث واحد (إما "login" أو "logout") مع الـ timestamp بتاعه.
# الجدول ده هو اللي بيمكّن صفحة Doctor Activity + قسم Live Activity في Overview.
# ════════════════════════════════════════════════════════════════════
class LoginHistory(Base):
    ...
    event_type = Column(String, nullable=False)  # "login" | "logout"

    # توقيت الحدث (UTC)
    timestamp = Column(DateTime(timezone=True), server_default=func.now(), index=True)

    # IP address اختياري (لو حابب تجمعه من request.client.host لاحقاً)
    ip_address = Column(String, nullable=True)

    # علاقة مع جدول الدكاترة
    doctor = relationship("Doctor", back_populates="login_history")